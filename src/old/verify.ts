...
const getPdApi = (
  endpoint: string = KusamaTestnetEndpoint
): Promise<ApiPromise> => {
...
export const verifyGenesis = async (cmd: Command) => {
  ...
  console.log(
    `You are connected to ${chain} using ${nodeName} v${nodeVersion}`
  );
  ...
  claimers.forEach(async (value: any, key: any) => {
    ...
    if (pdBalance.toString() !== balance.toString()) {
      throw new Error(
        `Balance | Got: ${pdBalance.toString()} | Expected ${balance.toString()}`
      );
    } else {
    ...
    if (vested.toNumber() > 0) {
      ...
      if (vested.toNumber() !== pdVested.locked) {
        throw new Error(
          `Vesting | Got: ${pdVested.locked} | Expected: ${vested.toNumber()}`
        );
      } else {
