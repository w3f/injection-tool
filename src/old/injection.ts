...
export const injectAllocations = async (
    ...
    while (i < addresses.length) {
        ...
        const tx = Object.assign(txParams, { data: encoded, to: frozenToken.options.address, nonce: startingNonce + i });
...
export const injectAmends = async (
    ...
    for (let i = 0, end = step; i < amends.length; i += step, end = Math.min(end + step, amends.length)) {
        ...
        const tx = Object.assign(txParams, { data: encoded, to: claims.options.address, nonce: startingNonce + nonceCounter });
...
export const injectClaims = async (
    ...
    while (i < eths.length) {
        ...
        const tx = Object.assign(txParams, { data: encoded, to: claims.options.address, nonce: startingNonce + i });
