...
export const injectAllocations = async (
  ...
  while (i < addresses.length) {
    if (noisy) {
      console.log(
        `Sending transfer of ${balances[i]} FrozenToken to ${addresses[i]}`
      );
    }

    const encoded = frozenToken.methods
      .transfer(addresses[i], balances[i])
      .encodeABI();
    const tx = Object.assign(txParams, {
      data: encoded,
      to: frozenToken.options.address,
...
export const injectAmends = async (
  ...
  for (
    let i = 0, end = step;
    i < amends.length;
    i += step, end = Math.min(end + step, amends.length)
  ) {
    if (noisy) {
...
export const injectClaims = async (
  ...
  if (eths.length != pubKeys.length) {
    throw new Error(
      "ERROR: provided args `eths` and `pubKeys` as arrays of different lengths."
    );
  }
