...
export const getClaimsContract = (
  w3: any,
  claimsAbi: any = ClaimsAbi,
  address: string = ClaimsAddress
): any => new w3.eth.Contract(claimsAbi, address);

export const getFrozenTokenContract = (
  w3: any,
  frozenTokenAbi: any = FrozenTokenAbi,
  address: string = FrozenTokenAddress
): any => new w3.eth.Contract(frozenTokenAbi, address);

export const getTokenHolderData = async (
  frozenTokenContract: any,
  claimsContract: any,
  atBlock = "latest"
): Promise<Map<EthAddress, ClaimData>> => {
  ...
  (
    ...
    assert(
      ...
      `Claimed: Account ${eth} not found having balance!`
    );
  ...
  (
    ...
    assert(
      tokenHolders.has(eth),
      `Vested: Account ${eth} not found having balance!`
    );
...
export const writeGenesis = async (cmd: Command) => {
  ...
  Template.genesis.runtime.staking.stakers.forEach((entry: any) => {
    const [account1, account2, reqBalance] = entry;
    Template.genesis.runtime.balances.balances.push(
      [account1, reqBalance],
      [account2, reqBalance]
    );
