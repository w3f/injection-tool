...
export const getAmended = async (claims: Contract, fromBlock: string = '0', toBlock: string = 'latest') => (await claims.getPastEvents('Amended', {
    fromBlock,
    toBlock,
})).map((event: any) => ({
...
export const getClaimedFromEvents = async (claims: Contract, fromBlock: string = '0', toBlock: string = 'latest') => (await claims.getPastEvents('Claimed', {
    fromBlock,
    toBlock,
})).map((event: any) => ({
...
export const getVestedFromEvents = async (claims: Contract, fromBlock: string = '0', toBlock: string = 'latest') => (await claims.getPastEvents('Vested', {
    fromBlock,
    toBlock,
...
export const writeGenesis = (memory: Map<DotPublicKey, ClaimData>, template: any, stillToClaim: any[]) => {
    ...
    indices.sort((a: IndexData, b: IndexData) => Number(a.index) - Number(b.index));
