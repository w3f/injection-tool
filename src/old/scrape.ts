...
export const getAmended = async (
  claims: Contract,
  fromBlock = "0",
  toBlock = "latest"
) =>
  (
    await claims.getPastEvents("Amended", {
      fromBlock,
      toBlock,
    })
  ).map((event: any) => ({
...
export const getClaimedFromEvents = async (
  claims: Contract,
  fromBlock = "0",
  toBlock = "latest"
) =>
  (
    await claims.getPastEvents("Claimed", {
      fromBlock,
      toBlock,
    })
  ).map((event: any) => ({
...
export const getVestedFromEvents = async (
  claims: Contract,
  fromBlock = "0",
  toBlock = "latest"
) =>
  (
    await claims.getPastEvents("Vested", {
      fromBlock,
      toBlock,
    })
  ).map((event: any) => event.returnValues.eth);

export const getFullDataFromState = async (
  claims: Contract,
  frozenToken: Contract
) => {
  ...
  for (let i = 0; i < Number(claimedLength); i++) {
    ...
    const { index, pubKey, vested } = await claims.methods
      .claims(ethAddress)
      .call();
  ...
  const stillToClaim = await Promise.all(
    Array.from(allHolders).map(async (holder: EthAddress) => {
...
export const writeGenesis = (
  memory: Map<DotPublicKey, ClaimData>,
  template: any,
  stillToClaim: any[]
) => {
  ...
  indices.sort(
    (a: IndexData, b: IndexData) => Number(a.index) - Number(b.index)
  );
