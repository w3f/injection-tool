...
export const injectKusamaState = async (cmd: Command) => {
  ...
  const input = fs.readFileSync(csv, { encoding: 'utf-8' }).split('\n').reverse().map((line) => {
    const [whomAndExtraData, almostCompleteJson] = line.split(',{');
    return {
      whom: whomAndExtraData.split(',')[0],
  ...
  const startingNonce = await api.query.system.accountNonde(sudoSigner.address);
  ...
  for (const entry of input) {
    ...
    if (util.u8aToHex(callIndex) === CallIndices.Claim) {
      //@ts-ignore
