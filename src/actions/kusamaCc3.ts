...
import { createType, GenericImmortalEra, GenericCall } from '@polkadot/types';
...
const main = async (cmd: any) => {
  ...
  for (const line of input) {
    ...
    if (json.kind == 2) {
      ...
      try {
        let { moduleId, callId, sender, params} = json;
        if (callId === 'validate') {
      ...
      if (moduleId === 'claims' && callId === 'claim') {
        ...
        if (moduleId === 'sudo' && callId === 'sudo_as') {
          let [who,params] = JSON.parse(json.params);
          let values = params.value.params.map((entry:any) => {
            if (entry.type === "Address") {
              return encodeAddress('0x' + entry.value, 2);
            } else if (entry.type.indexOf('<BalanceOf>') !== -1) {
          ...
          if (method === 'nominate') {
            values[0] = values[0].map((value: any) => {
              return encodeAddress('0x' + value, 2)
            });
          }
          if (method === 'setKeys') {
          ...
          let values = params.map((entry:any) => {
            if (entry.type === "AccountId") {
              return encodeAddress(entry.value, 2);
            }
            if (entry.type === "Address") {
              return encodeAddress('0x' + entry.value, 2);
            } else if (entry.type.indexOf('<BalanceOf>') !== -1 || entry.type.indexOf('Balance')) {
              if (entry.value.validatorPayment) {
                const payment = util.hexToBn(util.numberToHex(entry.value.validatorPayment)).toString();
                if (payment != '0') {
                  return { commisssion: 1000000000 };
          ...
          let values = JSON.parse(json.params).map((entry:any) => {
            if (entry.type === "Address") {
              return encodeAddress('0x' + entry.value, 2);
            } else if (entry.type.indexOf('<BalanceOf>') !== -1) {
              if (entry.value.validatorPayment) {
