...
export const makeAmendments = async (cmd: Command) => {
  ...
  let step = Math.min(50, originals.length);

  const startingNonce = utils.hexToNumber(await api.parity.nextNonce(txParams.from));
  let nonceCounter = 0;

  let start = 0;
  for (let i = start, end = step; i < originals.length; i += step, end = Math.min(end + step, originals.length)) {
    ...
    const encoded = claimsContract.methods.setVesting(originalsArg, amendsArg).encodeABI();
