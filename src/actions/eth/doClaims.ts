...
export const doClaims = async (cmd: Command) => {
  ...
  for (let i = 0; i < destinations.length; i++) {
    console.log(`Sending claim for allocation at ${destinations[i]} for pubkey ${pubKeys[i]} with nonce ${startingNonce + i}`);
  
    const encoded = claimsContract.methods.claim(destinations[i], pubKeys[i]).encodeABI();
