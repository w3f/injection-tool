...
export const validateAddress = async (cmd: Command) => {
  ...
  if (destinations.length != amounts.length) {
    throw new Error(
      "Attempted to supply arrays of non-equal lengths to `validateAddress`!"
    );
  }
  ...
  for (let i = 0; i < destinations.length; i++) {
    const isDOTHolder: any = await claimsContract.methods
      .hasAllocation(destinations[i])
      .call();

    if (isDOTHolder) {
      try {
        const result = await claimsContract.methods
          .setVesting([destinations[i]], [amounts[i]])
          .estimateGas({ from: from });
        console.log(
          `Address ${destinations[i]} has not called setVesting before`
        );
      } catch (err) {
